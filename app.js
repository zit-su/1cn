...
let activeScreen = 'splashScreen'; // Start on splash screen
...
auth.onAuthStateChanged(async (user) => {
    const minSplashTime = 1500; // in ms
...
function renderGroupMemberResults(results) {
    ...
    results.forEach(user => {
        ...
        div.innerHTML = `
            ...
            <button class="user-action-btn ${isSelected ? 'friend' : 'add'}" onclick="toggleGroupMember('${user.uid}')">
