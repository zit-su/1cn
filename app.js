...
function showScreen(screenId) {
    const screens = ['splashScreen','loginScreen','homeScreen','newChatScreen','createGroupScreen','userInfoScreen','chatScreen'];
    screens.forEach(id => {
...
function switchLoginTab(tab) {
    ...
    errorEl.classList.remove('visible'); errorEl.style.display = 'none';
    if (tab === 'login') {
...
function showLoginError(message) {
    ...
    setTimeout(() => { errorEl.classList.remove('visible'); setTimeout(()=>errorEl.style.display='none',400); }, 3000);
}
...
async function handleLogin() {
    ...
    } catch (error) { showLoginError(getAuthErrorMessage(error)); }
    finally { hideLoading(); }
}
async function handleRegister() {
    ...
    try {
        ...
        await db.ref('users/' + result.user.uid).set({
            name, email, bio:'Hey there! I am using ChatVerse.',
...
async function handleLogout() {
    try {
        await auth.signOut();
        deleteCookie('chatverse_user_email'); deleteCookie('chatverse_user_uid'); deleteCookie('chatverse_user_name');
        currentUser = null; currentUserData = null; friendsCache = {}; chatsCache = {};
...
// Auth State Listener with splash
auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
        currentUser = user;
        if (!getCookie('chatverse_user_uid')) {
        ...
        try {
            ...
            setTimeout(() => { hideLoading(); showScreen('homeScreen'); renderHomeChats(); }, delay);
        } catch (error) {
            console.error(error);
            const elapsed = Date.now() - startTime;
            const delay = Math.max(0, minSplashTime - elapsed);
            setTimeout(() => { hideLoading(); showScreen('loginScreen'); showToast('Error loading data', 'error'); }, delay);
        }
    } else {
        const elapsed = Date.now() - startTime;
        const delay = Math.max(0, minSplashTime - elapsed);
        setTimeout(() => { hideLoading(); showScreen('loginScreen'); }, delay);
    }
});
...
async function loadFriends() {
    ...
    try {
        ...
        for (const uid of Object.keys(friendsCache)) {
...
async function loadChats() {
    if (!currentUser) return;
    try {
        const snap = await db.ref('chats').orderByChild('participants/' + currentUser.uid).equalTo(true).get();
        chatsCache = {};
        const data = snap.val();
        if (data) {
            for (const [chatId, chat] of Object.entries(data)) {
                ...
                if (chat.type === 'direct') {
                    ...
                    if (otherUid && !allUsersCache[otherUid]) {
                        const uSnap = await db.ref('users/' + otherUid).get();
                        if (uSnap.exists()) allUsersCache[otherUid] = { uid: otherUid, ...uSnap.val() };
                    }
                }
            }
        }
...
function listenToChatUpdates() {
    ...
    chatListeners['chat_list'] = db.ref('chats').orderByChild('participants/' + currentUser.uid).equalTo(true)
        .on('value', async (snap) => {
            chatsCache = {};
            const data = snap.val();
            if (data) {
...
async function searchUsers(query) {
    ...
    try {
        ...
        emptyState.style.display = results.length === 0 ? 'flex' : 'none';
        results.forEach(user => resultsList.appendChild(createUserListItem(user)));
    } catch (e) { emptyState.style.display = 'flex'; }
    finally { hideLoading(); }
}
function renderFriendsList() {
    ...
    emptyState.style.display = friendUids.length === 0 ? 'flex' : 'none';
...
function createUserListItem(userData, isFriend=false) {
    ...
    let actionBtn = '';
    if (isFriend || friendsCache[userData.uid]) {
    ...
    div.innerHTML = `
        ...
        <div class="user-info"><div class="user-name">${userData.name||'User'}</div><div class="user-email">${userData.email||''}</div></div>
        ${actionBtn}
    `;
...
async function addFriend(friendUid) {
    ...
    try {
        ...
        if (userSearchFilter) searchUsers(userSearchFilter); else renderFriendsList();
    } catch (e) { showToast('Error adding friend', 'error'); }
    finally { hideLoading(); }
}
...
function showUserInfo(uid) {
    ...
    if (isFriend) {
        ...
        actionsEl.innerHTML = `<button class="btn btn-primary" onclick="addFriend('${uid}')"><i class="fas fa-user-plus"></i> Add Friend</button>`;
    }
...
function showCurrentUserInfo() {
    if (currentUser && currentUserData) {
        ...
        allUsersCache[uid] = { uid, ...currentUserData };
        const avatarColor = currentUserData.avatarColor || getAvatarColor(uid);
        document.getElementById('profileAvatar').textContent = getInitials(currentUserData.name);
        document.getElementById('profileAvatar').style.background = avatarColor;
        document.getElementById('profileName').textContent = currentUserData.name || 'You';
        document.getElementById('profileEmail').textContent = currentUserData.email || '';
        document.getElementById('profileBio').textContent = currentUserData.bio || 'No bio yet';
        document.getElementById('profileSince').textContent = currentUserData.createdAt ? new Date(currentUserData.createdAt).toLocaleDateString() : 'Unknown';
...
function renderHomeChats() {
    ...
    const filtered = Object.entries(chatsCache).filter(([id, chat]) => {
    ...
    filtered.sort((a,b) => (b[1].lastMessageTime||0) - (a[1].lastMessageTime||0))
...
function getChatDisplayName(chat) {
    ...
    const otherUid = Object.keys(chat.participants).find(uid => uid !== currentUser?.uid);
    return allUsersCache[otherUid]?.name || 'Chat';
}
function createChatListItem(chatId, chat) {
    ...
    if (chat.type === 'group') {
        ...
        const otherUid = Object.keys(chat.participants).find(uid => uid !== currentUser?.uid);
        const userData = allUsersCache[otherUid] || { name:'User' };
...
async function searchGroupMembers(query) {
    ...
    } catch(e) { renderGroupMemberResults([]); }
    finally { hideLoading(); }
}
function renderGroupMemberResults(results) {
    ...
    container.innerHTML = '';
    if (results.length === 0) {
        container.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:16px;">Search to add members</p>';
        return;
    }
    results.forEach(user => {
        ...
        div.innerHTML = `
            ...
            <div class="user-info"><div class="user-name">${user.name||'User'}</div><div class="user-email">${user.email||''}</div></div>
            <button class="user-action-btn ${isSelected?'friend':'add'}" onclick="toggleGroupMember('${user.uid}')">${isSelected?'✓ Added':'+ Add'}</button>
        `;
        container.appendChild(div);
    });
}
function toggleGroupMember(uid) {
    selectedGroupMembers.has(uid) ? selectedGroupMembers.delete(uid) : selectedGroupMembers.add(uid);
    renderSelectedMembers();
...
async function createGroup() {
    ...
    try {
        ...
        await chatRef.set({
            type:'group', participants, groupName, createdBy:currentUser.uid,
            createdAt:firebase.database.ServerValue.TIMESTAMP,
            lastMessage:'Group created', lastMessageTime:firebase.database.ServerValue.TIMESTAMP, lastSenderUid:currentUser.uid
        });
...
async function startDirectChat(otherUid) {
    ...
    try {
        ...
        if (!chat) {
            ...
            if (snap.exists()) { chat = snap.val(); chatsCache[chatId] = chat; }
            else {
                chat = { type:'direct', participants:{[currentUser.uid]:true,[otherUid]:true}, createdAt:firebase.database.ServerValue.TIMESTAMP, lastMessage:'', lastMessageTime:firebase.database.ServerValue.TIMESTAMP, lastSenderUid:'' };
...
function openChat(chatId, chatData) {
    ...
    document.getElementById('chatHeaderName').textContent = getChatDisplayName(chatData);
    ...
    if (chatData.type === 'group') { avatarEl.innerHTML = '<i class="fas fa-users"></i>'; avatarEl.style.background = '#7C3AED'; }
    else {
    ...
    if (chatListeners['messages_' + chatId]) { chatListeners['messages_' + chatId](); delete chatListeners['messages_' + chatId]; }
    chatListeners['messages_' + chatId] = db.ref('messages/' + chatId).orderByChild('timestamp').on('value', snap => renderMessages(snap.val()));
...
function renderMessages(messagesData) {
    ...
    messages.forEach(msg => {
        ...
        row.innerHTML = `<div class="message-bubble">${senderName?`<span class="message-sender">${senderName}</span>`:''}${msg.text||''}<span class="message-time">${formatMessageTime(msg.timestamp)}</span></div>`;
        container.appendChild(row);
    });
    scrollChatToBottom();
}
function formatMessageTime(ts) { if (!ts) return ''; return new Date(ts).toLocaleTimeString('en-US', { hour:'2-digit', minute:'2-digit' }); }
function scrollChatToBottom() { const el=document.getElementById('messagesContainer'); if(el) el.scrollTop=el.scrollHeight; }
...
async function sendMessage() {
    ...
    try {
        ...
        await messageRef.set({ senderUid: currentUser.uid, text, timestamp: firebase.database.ServerValue.TIMESTAMP });
        await db.ref('chats/' + currentChatId).update({ lastMessage:text, lastMessageTime:firebase.database.ServerValue.TIMESTAMP, lastSenderUid:currentUser.uid });
        scrollChatToBottom();
    } catch(e) { showToast('Failed to send message', 'error'); input.value=text; document.getElementById('sendBtn').disabled=false; }
}
function closeChatScreen() {
    if (chatListeners['messages_' + currentChatId]) { chatListeners['messages_' + currentChatId](); delete chatListeners['messages_' + currentChatId]; }
    currentChatId = null; currentChatData = null; currentOtherUserUid = null;
    showScreen('homeScreen'); renderHomeChats();
}
// Init
(function() {
...
console.log('💬 ChatVerse initialized');
